...
export default function InProgressTasksPage() {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        ...
        const inProgressTasks = tasksData.filter((task: any) => task.task_status === 'in_progress');
        setTasks(inProgressTasks);
      } catch (error) {
        console.error('Error fetching tasks:', error);
        setError('Failed to load in-progress tasks. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      ...
      // Remove task from list if status is changed from in_progress
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-8">
          ...
          <h3 className="text-lg font-medium mb-1">No in-progress tasks found</h3>
